...
Update includes removal of letters after guessed
...
def restart():
    ...
    for letter in reference:
        Rrelx = letter[1]
        Rrely = letter[2]
        variable = eval(letter[0])
        variable.place(relx=Rrelx, rely=Rrely)                
...
def check(guess):
    ...
    info = guess.place_info()
    relx = info['relx']
    guesses.append(relx)
    rely = info['rely']
    guesses.append(rely)
    reference.append(guesses)
    guess.place_forget()
